...
namespace FFRadarBuddy
{
    ...
    public class JsonParser
    {
        ...
        public class IntValue : FloatValue
        {
            public int IntNumber;

            public IntValue(int Num) : base(Num * 1.0f) { IntNumber = Num; }
            public override string ToString() { return IntNumber.ToString(); }
            public static implicit operator int(IntValue v) => v.IntNumber;
            public static new IntValue Empty = new IntValue(0);
        }
        ...
        public class FloatValue : Value
        {
            ...
            public FloatValue(float Num) { FloatNumber = Num; }
            public override string ToString() { return FloatNumber.ToString(); }
            public static implicit operator float(FloatValue v) => v.FloatNumber;
            ...
            for (int Idx = 0; Idx < jsonStr.Length; Idx++)
            {
                ...
                if (jsonStr[Idx] == ',' || jsonStr[Idx] == ']' || jsonStr[Idx] == '}')
                {
                    if (activeValue == null)
                    {
                        ...
                        if (strCurrent.Length > 0 || bHasStringValue)
                        {
                            if (bHasStringValue)
                            {
